...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['generate_bill'])) {
        ...
        $net = max($total_amount - $discount, 0);
        $balance = $net - $paid;
        $status = ($balance <= 0) ? 'open' : 'draft';

        $stmt = $conn->prepare("UPDATE billing SET total_amount=?, discount=?, paid_amount=?, balance_amount=?, status=? WHERE billing_id=?");
        $stmt->bind_param("ddddsi", $net, $discount, $paid, $balance, $status, $billing_id);
        if ($stmt->execute()) {
    ...
    if ($patientRow && !empty($patientRow['referred_by'])) {
        ...
        // Total test price for this bill
        $commission_base = 0;
        $stmt2 = $conn->prepare("SELECT t.price FROM test_assignments ta 
                                 JOIN tests t ON ta.test_id = t.test_id 
                                 WHERE ta.billing_id = ?");
        ...
        while ($row = $result->fetch_assoc()) {
            $commission_base += $row['price'];
        }
        $stmt2->close();

        // Get doctor commission rate
        $rateRow = $conn->query("SELECT commission_percent FROM doctors WHERE doctor_id = $doctor_id")->fetch_assoc();
        $commission_percent = $rateRow['commission_percent'] ?? 10;
        $commission_amount = ($commission_percent / 100) * $commission_base;

        // Insert into doctor_commissions table
        $stmt3 = $conn->prepare("INSERT INTO doctor_commissions (doctor_id, billing_id, commission_amount) 
                                 VALUES (?, ?, ?)");
    ...
    if (isset($_POST['pay_due'])) {
        ...
        $paid = $active_billing['paid_amount'] + $payment;
        $balance = max($active_billing['total_amount'] - $paid, 0);
        $status = ($balance <= 0) ? 'open' : $active_billing['status'];

        $stmt = $conn->prepare("UPDATE billing SET paid_amount=?, balance_amount=?, status=? WHERE billing_id=?");
        $stmt->bind_param("ddsi", $paid, $balance, $status, $billing_id);
        if ($stmt->execute()) {
            header("Location: billing.php?patient_id=$patient_id&billing_id=$billing_id&toast=1");
            exit();
        }
...
<div class="container mt-4">
    <div class="card">
        <div class="card-header bg-primary text-white"><h4 class="mb-0">🧾 Patient Billing</h4></div>
        <div class="card-body">
                ...
                <div class="form-group mb-3">
                    ...
                    <select onchange="window.location.href=this.value" class="form-control">
                        ...
                        while ($b = $bills->fetch_assoc()):
                            $badge = $b['status'] === 'draft' ? '🟡' : ($b['status'] === 'open' ? '🟢' : '📄');
                        ?>
            ...
            <?php if ($active_billing): ?>
                <div class="invoice-summary p-3 bg-white border rounded" id="print-area">
                    <h5><?= htmlspecialchars($patient['name']) ?>
                        <span class="badge <?= $active_billing['status'] == 'draft' ? 'badge-draft' : ($active_billing['status'] == 'open' ? 'badge-open' : 'badge-printed') ?>">
                            <?= ucfirst($active_billing['status']) ?>
                        </span>
                    </h5>
                    ...
                    <?php if (!empty($tests_array)): ?>
                        <table class="table table-bordered">
                            <thead class="thead-light"><tr><th>Test</th><th class="text-right">Price (₹)</th></tr></thead>
                            <tbody>
                                <?php foreach ($category_totals as $row): ?>
    <tr><td><?= htmlspecialchars($row['category_name']) ?></td><td class="text-right">₹ <?= number_format($row['total_price'], 2) ?></td></tr>
<?php endforeach; ?>



                                <tr class="bg-light"><td><strong>Subtotal</strong></td><td class="text-right">₹ <?= number_format($total_amount, 2) ?></td></tr>
                                <tr class="table-warning"><td><strong>Discount</strong></td><td class="text-right">₹ <?= number_format($active_billing['discount'], 2) ?></td></tr>
                                <tr class="table-info font-weight-bold"><td>Net Payable</td><td class="text-right">₹ <?= number_format($total_amount - $active_billing['discount'], 2) ?></td></tr>
                                <tr><td>Paid</td><td class="text-right">₹ <?= number_format($active_billing['paid_amount'], 2) ?></td></tr>
                                <tr><td>Due</td>
...
<script>
Swal.fire({ icon: 'success', toast: true, title: 'Billing updated!', position: 'top-end', timer: 3000, showConfirmButton: false });
</script>
<?php endif; ?>
</body>
</html>
