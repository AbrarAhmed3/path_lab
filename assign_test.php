...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action === 'new_visit') {
        $stmt = $conn->prepare("INSERT INTO billing (patient_id, total_amount, paid_amount, balance_amount, status, visit_note, referred_by) VALUES (?, 0, 0, 0, 'draft', 'New visit', ?)");
...
<head>
    ...
    <style>
        ...
        .card-header { background: linear-gradient(90deg, #28a745, #218838); padding: 1rem 1.25rem; }
        .card-header h4 { font-weight: 600; margin-bottom: 0; }
        .form-group label { font-weight: 600; }
        .form-control:focus, .select2-selection:focus { box-shadow: none; border-color: #007bff; }
        ...
        .badge-draft, .badge-open, .badge-printed {
            padding: 0.5em 0.8em; border-radius: 20px; font-size: 0.85rem; font-weight: 600;
        }
        .badge-draft { background-color: #ffc107; color: #212529; }
        .badge-open { background-color: #28a745; color: white; }
        .badge-printed { background-color: #17a2b8; color: white; }
        ul.assigned-tests { padding-left: 18px; margin-top: 0.5rem; margin-bottom: 1.2rem; }
        ul.assigned-tests li { font-weight: 500; margin-bottom: 3px; }
        hr { margin: 1.5rem 0; }
    </style>
</head>
<body>
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form method="GET" class="form-row align-items-end">
                ...
                <div class="form-group col-md-5">
                    ...
                    <select name="billing_id" class="form-control" onchange="this.form.submit()">
                        ...
                        <?php
                        $visits = $conn->query("SELECT billing_id, billing_date, status FROM billing WHERE patient_id = $patient_id ORDER BY billing_date DESC");
                        while ($v = $visits->fetch_assoc()):
                            $badge = $v['status'] == 'draft' ? '🟡' : ($v['status'] == 'open' ? '🟢' : '📄');
                        ?>
            ...
            <?php elseif ($billing_id && $current_billing): ?>
                <h5>
                    Status:
                    <span class="badge <?= $current_billing['status'] == 'draft' ? 'badge-draft' : ($current_billing['status'] == 'open' ? 'badge-open' : 'badge-printed') ?>">
                        <?= ucfirst($current_billing['status']) ?>
                    </span>
                </h5>
                ...
                <?php if ($current_billing['status'] === 'draft'): ?>
                    <form method="POST">
                        <input type="hidden" name="patient_id" value="<?= $patient_id ?>">
...
$(document).ready(function () {
    $('.js-select-patient').select2({ placeholder: "-- Select Patient --" });
    $('.js-select-tests').select2({ placeholder: "Select tests to assign", width: '100%', dropdownAutoWidth: true });
    $('.js-select-doctor').select2({ placeholder: "Select referring doctor", width: '100%' });
