...
// Fetch billing records only if patient is selected
$billings = [];
if ($patient_id > 0) {
...
$billing_status = null;
if ($billing_id > 0) {
    $status_query = $conn->prepare("SELECT status FROM billing WHERE billing_id = ?");
    $status_query->bind_param("i", $billing_id);
    $status_query->execute();
    $status_query->bind_result($billing_status);
    $status_query->fetch();
    $status_query->close();
}
...
// Fetch previously selected doctors
$prefill_doctors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $billing_status !== 'finished') {
    ...
    $newStatus = ($hasDoctor && $allMachinesFilled) ? 'finalized' : 'ready';

    $statusStmt = $conn->prepare("UPDATE billing SET status = ? WHERE billing_id = ?");
    $statusStmt->bind_param("si", $newStatus, $billing_id);
    $statusStmt->execute();
            ...
            <form method="POST">
                ...
                <?php if ($billing_status === 'finished'): ?>
                    <div class="alert alert-info">
                        <i class="fas fa-lock"></i> This report is marked as <strong>finished</strong>. Editing is disabled.
                    </div>
                <?php endif; ?>
                ...
                <div class="form-group">
                    ...
                    <?php while ($doc = $lab_doctors->fetch_assoc()): ?>
                        <div class="form-check form-check-inline mb-2">
                            <input class="form-check-input" type="checkbox" name="doctor_ids[]" id="doc_<?= $doc['doctor_id'] ?>" value="<?= $doc['doctor_id'] ?>"
                                <?= in_array($doc['doctor_id'], $prefill_doctors) ? 'checked' : '' ?> <?= ($billing_status === 'finished') ? 'disabled' : '' ?>>
                            <label class="form-check-label mr-3" for="doc_<?= $doc['doctor_id'] ?>"> <?= $doc['name'] ?> </label>

                            <label class="ml-2">
                                <input type="checkbox" name="treating_doctor_ids[]" value="<?= $doc['doctor_id'] ?>"
                                    <?= in_array($doc['doctor_id'], $prefill_treating_doctors) ? 'checked' : '' ?> <?= ($billing_status === 'finished') ? 'disabled' : '' ?>>
                    ...
                    <?php if ($departments && $departments instanceof mysqli_result): ?>
                        ...
                        ?>
                            <div class="form-inline mb-2">
                                ...
                                <input type="text" name="machine_info[<?= $dept_name ?>]" class="form-control w-50" value="<?= $value ?>" placeholder="Enter machine used" <?= ($billing_status === 'finished') ? 'readonly' : '' ?>>
                            </div>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <p class="text-muted">Select patient and billing to load department-specific machines.</p>
                    <?php endif; ?>
                </div>
...
<!-- JS: Auto update billing dropdown on patient change -->
<script>
